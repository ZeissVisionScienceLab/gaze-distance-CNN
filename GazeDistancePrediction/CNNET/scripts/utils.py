...
def standardize(data, train_indices, val_indices, test_indices):
    """
    Function to standardize eye tracking information and ipd.
    ...
    val_indices: np.array
        indices of the validation data
    test_indices: np.array
        indices of the test data
    return: 
        standardized data
    """
    categories = ['l_gaze.x', 'l_gaze.y', 'r_gaze.x', 'r_gaze.y', 'ipd']

    for category in categories:
        mean = np.mean(data.loc[train_indices, category].to_numpy())
        std = np.std(data.loc[train_indices, category].to_numpy())

        print(category)
        print('Mean of training data: ', mean)
        print('Std: of training data: ', std)

        print(data.loc[train_indices, category].to_numpy())

        data.loc[train_indices, category] = (data.loc[train_indices, category].to_numpy() - mean) / std
        data.loc[val_indices, category] = (data.loc[val_indices, category].to_numpy() - mean) / std
        data.loc[test_indices, category] = (data.loc[test_indices, category].to_numpy() - mean) / std
